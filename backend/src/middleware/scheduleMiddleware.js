...
export const checkSchedule = async (req, res, next) => {
    const client = await clientPool.connect();
    try {
        const teamId = req.params.teamId;
        const now = new Date();
        const teamInfo = await client.query("SELECT * FROM teams WHERE id=$1", [teamId]);
        if (teamInfo.rows.length == 0)
            throw new AppError("No information", 404);
        const teamData = teamInfo.rows[0];
        const timelocale = new Date(now.toLocaleString("en-us", { timeZone: teamData.timeZone }));
        ...
        if (currentTime >= openTime && currentTime <= closeTime) {
            ...
            return next(new AppError("Window is closed", 401));
        }
    }
    catch (error) {
        next(error);
